function [] = plot_sol_3D(t,y,T,species,chan_pos)
...
for j=species
    ...
    if j==6
        ...
        for k=chan_pos
            ...
            plot3(XTR,YTR,ynew)
            TITLE=['Specie ' num2str(j) ' - Theta' ];
            title(TITLE);
            xlabel('Time'), ylabel(['Washcoat Channel Position: ' num2str(k)]), zlabel('Theta')
            counter=counter+1;
        end   
    end
    ...
    if j==7
        ...
        subplot(1,2,1)
        plot3(XTZ,YTZ,ynew)
        TITLE='Bulk Energy Tf';
        title(TITLE);
        xlabel('Time'), ylabel('Channel'), zlabel('Tf')
        
        ynew=y(T,Twcs:Twcs:Twcs*Nc);
        subplot(1,2,2)
        plot3(XTZ,YTZ,ynew)
        TITLE='Solid Energy Ts';
        title(TITLE);
        xlabel('Time'), ylabel('Channel'), zlabel('Ts')
